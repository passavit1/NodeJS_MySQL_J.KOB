...
// สร้าง promisePool ควบคุมเวลา Query
...
async function insertNewItem(billSale, product) {
  var sql =
    "INSERT INTO tb_bill_sale_detail(product_id, bill_sale_id, price, cost, qty) VALUES(?, ?, ?, ?, ?)";
...
async function updateItem(billSale, product) {
  var sql =
    "UPDATE tb_bill_sale_detail SET qty = qty + 1 WHERE bill_sale_id = ? AND product_id = ?";
...
async function getItem(billSale, product) {
  var sql =
    "SELECT * FROM tb_bill_sale_detail WHERE bill_sale_id = ? AND product_id = ?";
  var params = [billSale.id, product.id];
  ...
  if (req.body.returnPrice < 0) {
    ...
    mysql.query(sql, (err, rs) => {
      if (err) {
        ...
        res.redirect(
          "billSale/" + req.body.inputPrice + "/" + req.body.returnPrice
        );
      }
    });
  }
...
router.get("/billSale/:inputPrice/:returnPrice", (req, res) => {
  ...
  var sql =
    "SELECT * FROM tb_bill_sale WHERE status = 'close' ORDER BY id DESC LIMIT 1";
